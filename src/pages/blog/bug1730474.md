...
When no ARIA roles are applied, a `<td>` element in an HTML table has the default role of `cell`. Notably, this role is absent from the "Used in Roles" values in the [ARIA specification](https://www.w3.org/TR/wai-aria-1.1/#aria-selected) page. This means that by default, the `aria-selected` state value for `cell` nodes in the [tree](https://developer.mozilla.org/en-US/docs/Glossary/Accessibility_tree) should be `undefined`. Having it `false` or `true` would imply they're selectable.
...
2. Creating a test in the a11y test suite that confirms this behavior has been fixed.
