...
When editing a large and complex codebase such as Mozilla Firefox, it is quite useful to be able to run some testing on your own. There is usually a test suite already in place for the section of the code that you're modifying, and by running these you can verify if your changes didn't break anything that was working before, i.e. do regression testing.

When I began tackling my first bugs, I didn't have [commit level 1](https://www.mozilla.org/en-US/about/governance/policies/commit/access-policy/) clearance. This meant I wasn't able to do a [try push](https://firefox-source-docs.mozilla.org/tools/try/index.html) and use Mozilla's CI testing system. I'd have to locally use the testing already built into the codebase.

According to the [official requirements](https://firefox-source-docs.mozilla.org/setup/linux_build.html), 4Gb of RAM is the bare minimum for an artifact build, but 8Gb recommended (at the time of this writing). It doesn't explicitly give the requirements for a non-artifact build, but I'd extrapolate - based on experience - 8Gb as the bare minimum and 16Gb as recommended.

## Artifact vs Non-artifact Build

Given that Mozilla Firefox's codebase is so vast and heavy on the hardware, the developers provide a lighter [artifact version](https://firefox-source-docs.mozilla.org/contributing/build/artifact_builds.html) of it. The difference is that this version comes with all C++ code pre-compiled, whereas in the non-artifact version the C++ is compiled locally, during the build. It is significantly faster to download all C++ already compiled - the artifacts - than to compile them locally during the build. 

However, for those making changes to C++ code there's no way around the non-artifact version. I realized this in practice, as I wasn't able to test my C++ code changes with the artifact build. The downloaded artifacts - compiled C++ - obviously didn't account for any local changes, and the only way to make them do it was to compile them locally.


## Digital Ocean's Droplets

At this time it would be much more convenient for me to **rent** powerful computing resources, than to outright **buy** them, i.e. to upgrade my hardware. The pricing from Digital Ocean's Virtual Private Servers (droplets) is geared towards a different use-case, and quite affordable if running for several hours, or even for a few days.

There's a myriad of choices for droplet specs, and I chose the cheapest with shared CPU that'd come with 16Gb of RAM. Upon creating it, you're immediately given its IP address that you can use to SSH into it. You ought to have provided your public SSH key to DO already (probably when creating your account). If you don't have, or want to generate a new SSH key, run `ssh-keygen` in your terminal. You might already have one at `~/.ssh/id_rsa.pub` though. Some people prefer having multiple SSH keys for different services (like GitHub).
...
Now to run the tests all I needed was to prepend the commands with `xvfb-run`. So for browser tests, for instance, I'd run, for example: `xvfb-run ./mach test accessible/tests/browser/states/` and likewise for mochitests: `xvfb-run ./mach test accessible/tests/mochitest/table/`.
...
With the remote and local codebases synced, run `./mach build` on the remote, non-artifact one, and run your tests. More often than not, you'll need to repeat the process because the attempt failed. Easy-peasy, just make sure you run `git restore . && git clean -fd` on the VPS to have the codebase back to its original state.

To summarize, these are the steps to run each attempt:
...
You then iterate these steps until no more code changes are needed. Remember to also use `git pull` on both local and remote codebases every few hours to keep them in sync with the official codebase.

## Takeaways

What I'd normally spend in a full day of work with the 16Gb of RAM droplet would be about $1.13 USD. And that with the droplet up and running for 10 hours or more. As long as I wouldn't forget to destroy the droplet, the amount spent was pretty negligible. I liked the pricing transparency and never felt overcharged.
